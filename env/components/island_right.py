...
class IslandRight(Island): 
    def __init__(self, view_engine): 
        ...
        self.height = self.state.island_right_height
        self.ascending = self.state.island_right_ascending
    ...
    def render(self, pos:tuple): 
        (x, y) = pos
        y += int(self.height)
        self.state.island_right_height = self.increment_height(self.height, self.ascending, self.setAscending)
        self.view_engine.display_vector(self.vector, x, y)
    
    def setAscending(self): 
        self.state.island_right_ascending = not self.state.island_right_ascending